...
## Snapshot
- WXT browser extension (Chrome/Firefox) that injects AniList/AniChart overlays and talks to Sonarr.
...
- `anilist-anime.content/` – MediaActions on AniList anime detail pages.
- `anilist-browse.content/` – overlays on AniList browse/search.
- `anichart-browse.content/` – overlays on AniChart browse.
- `options/` – options UI (Sonarr, mappings, UI, advanced).

## Key paths
- RPC contracts: `src/rpc/index.ts`, `src/rpc/schemas.ts`
- RPC handlers: `src/rpc/handlers/*`
- Clients: `src/clients/anilist/`, `src/clients/sonarr.api.ts`
- Services: `src/services/mapping/*`, `src/services/anilist/*`, `src/services/library/sonarr/*`
...
- UI overlays/components: `src/features/media-overlay/*`, `src/shared/components/*`
