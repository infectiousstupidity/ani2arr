...
export function normalizeProviderConnectionInput(
  ...
  if (!url || !apiKey) {
    throw new Error(`${label}: enter both URL and API key, or leave both blank.`);
  }
  ...
  const permissionPattern = getProviderHostPermissionPattern(normalizedUrl.value);
  if (!permissionPattern.ok) {
    throw new Error(`Failed to update ${label} host permissions. Please try again.`);
  }
...
export function normalizeProviderConnectionSettings(
  ...
  return normalizeProviderConnectionInput(getProviderConnectionDraft(settings, provider), provider);
}
