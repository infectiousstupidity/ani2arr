...
import { parseAniListIdOrNull, type AniListId } from "@/anilist/anilist-id";

export type SectionId = "sonarr" | "radarr" | "mappings" | "ui" | "advanced";
export type SectionGroup = "providers" | "extension";
...
export const sections: SectionConfig[] = [
	...
	},
	{
		...
		description: "Manage AniList manual mappings.",
...
export const navGroups: Array<{ title: string; group: SectionGroup }> = [
	{ title: "Providers", group: "providers" },
...
export const getSectionUrlHash = (
	...
	if (sectionId === "mappings" && targetAnilistId !== null) {
...
export const extractTargetAnilistIdFromHash = (
	hash: string,
): AniListId | null => {
	const cleaned = (hash ?? "").replace(/^#/, "");
	...
	if (!raw) return null;
	return parseAniListIdOrNull(Number(raw));
};
