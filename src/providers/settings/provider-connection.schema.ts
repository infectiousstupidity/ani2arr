...
export function validateProviderConnectionUrl(input: string): ProviderConnectionResult<string> {
...
export function validateProviderConnectionApiKey(input: string): ProviderConnectionResult<string> {
  const trimmed = input.trim();
  const parsed = v.safeParse(ApiKeySchema, trimmed);

  if (!parsed.success) {
    const first = parsed.issues?.[0];
    return { ok: false, error: (first && String(first.message)) || 'Invalid API key.' };
  }
