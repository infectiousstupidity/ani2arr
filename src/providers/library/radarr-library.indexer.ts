...
// src/providers/library/radarr-library.indexer.ts
...
import type { RadarrMovieSnapshot, TmdbId } from "@/providers";
import { LOCAL_INDEX_ACCEPTANCE_THRESHOLD } from "./constants";

export class RadarrLibraryIndexer {
	private tmdbSet: Set<TmdbId> = new Set();
	private normalizedTitleIndex: Map<string, TmdbId | null> = new Map();
	private moviesByTmdbId: Map<TmdbId, RadarrMovieSnapshot> = new Map();
	...
	findTmdbIdInIndex(
		payload: Pick<StatusInput, "title" | "metadata">,
	): TmdbId | null {
		...
		let bestMatch: { tmdbId: TmdbId; score: number } | null = null;

		const scoreAgainstMovie = (
			rawTitle: string,
			movie: RadarrMovieSnapshot,
		): number => {
		...
		for (const rawTitle of candidateInputs) {
			...
			for (const key of normalizedVariants) {
				...
				if (typeof match === "number" && this.tmdbSet.has(match)) {
					...
					if (
						score >= LOCAL_INDEX_ACCEPTANCE_THRESHOLD &&
	...
	private indexMovie(movie: RadarrMovieSnapshot): void {
		...
		const keys = this.normalizeTitleCandidates(
			extractCandidateTitleVariants("radarr", movie).map(
	...
	private normalizeTitleCandidates(
