...
// src/providers/library/sonarr-library.indexer.ts
...
import type { SonarrSeriesSnapshot, TvdbId } from "@/providers";
import { LOCAL_INDEX_ACCEPTANCE_THRESHOLD } from "./constants";

export class SonarrLibraryIndexer {
	private tvdbSet: Set<TvdbId> = new Set();
	private normalizedTitleIndex: Map<string, TvdbId | null> = new Map();
	private seriesByTvdbId: Map<TvdbId, SonarrSeriesSnapshot> = new Map();
	...
	findTvdbIdInIndex(
		...
		let bestMatch: { tvdbId: TvdbId; score: number } | null = null;

		const scoreAgainstSeries = (
			rawTitle: string,
	...
	private indexSeries(series: SonarrSeriesSnapshot): void {
		...
		const keys = this.normalizeTitleCandidates(
			extractCandidateTitleVariants("sonarr", series).map(
