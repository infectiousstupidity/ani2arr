...
describe("Seerr connection recovery", () => {
	...
	it.each([
		...
		expect(recover([code])).toEqual({
			label: "Reconnect Seerr",
			enableCsrf: false,
		});
	});

	it.each([ErrorCode.CONFIGURATION_ERROR, ErrorCode.PERMISSION_ERROR])(
		"opens settings for %s",
		(code) => {
			expect(recover([code])).toEqual({
				label: "Open Seerr settings",
				enableCsrf: false,
			});
		},
	);
	...
	it.each([
		...
		"chooses CSRF recovery for %s auth mode",
		(authMode, label, enableCsrf) => {
	...
	it("keeps reconnect priority when more than one error exists", () => {
		expect(
			recover([ErrorCode.SEERR_CSRF_REQUIRED, ErrorCode.SEERR_AUTH_REQUIRED]),
