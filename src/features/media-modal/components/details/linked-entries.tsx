...
// src/features/media-modal/components/details/linked-entries.tsx
...
export function MappingLinkedEntries(props: MappingLinkedEntriesProps): React.JSX.Element | null {
  ...
  const shouldFetchMetadata = entries.length === 0 && linkedAniListIds.length > 0;
  const uniqueLinkedIds = shouldFetchMetadata ? [...new Set(linkedAniListIds.filter(Boolean))] : [];
  ...
  const hasCurrentEntry = rows.some((row) => row.anilistId === currentAniListId || row.relation === 'current');

  return (
    <section className={className ?? 'space-y-2'}>
      <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-text-secondary">
        {hasCurrentEntry ? 'Linked AniList entries' : `Also linked AniList entr${rows.length === 1 ? 'y' : 'ies'}`}
      </p>
