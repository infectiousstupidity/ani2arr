...
// src/features/media-modal/details/details-tab.tsx
...
function addProviderStatRows(rows: MappingDetailRow[], mapping: MediaModalTargetSummary): void {
  if (mapping.provider === 'sonarr') {
    ...
    if (mapping.episodeFileCount !== undefined) {
      rows.push({ label: 'Episode files', value: String(mapping.episodeFileCount) });
    }
...
export function MappingPreviewDetails(props: MappingPreviewDetailsProps): React.JSX.Element {
  ...
  if (effectiveMapping) {
    ...
    addProviderStatRows(detailRows, effectiveMapping);
