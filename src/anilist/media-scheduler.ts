...
// src/anilist/media-scheduler.ts
...
export class AniListMediaScheduler {
	...
	private readonly pendingByPriority = new Map<
		RequestPriority,
		Map<AniListId, PendingEntry>
	>(
		PRIORITIES.map((priority) => [
			priority,
			new Map<AniListId, PendingEntry>(),
		]),
	);
	...
	public async requestMedia(
		...
		const settled = await Promise.allSettled(
			[...pending.entries()].map(
				async ([queuedId, promise]) => [queuedId, await promise] as const,
			),
		);
	...
	private refreshInBackground(
		...
		void this.ensureNetworkRequest(id, {
			...options,
			forceRefresh: true,
	...
	private ensureNetworkRequest(
		...
		this.bumpBucketReadyAt(
			options.priority,
			Date.now() + COALESCE_MS[options.priority],
		);
	...
	private promotePendingEntry(
		entry: PendingEntry,
		nextPriority: RequestPriority,
	): void {
		...
		this.bumpBucketReadyAt(
			nextPriority,
			Date.now() + COALESCE_MS[nextPriority],
		);

		if (import.meta.env.DEV) {
			this.log.debug?.(
				`anilist:scheduler promote id=${entry.id} priority=${nextPriority} from=${previousPriority}`,
			);
		}
	...
	private pickNextChunk(
		now = Date.now(),
	): { priority: RequestPriority; entries: PendingEntry[] } | null {
		for (const priority of PRIORITIES) {
			const bucket = this.pendingByPriority.get(priority);
			if (!bucket || bucket.size === 0) continue;
			...
			if (readyAt > now) {
				if (
					import.meta.env.DEV &&
					priority === "low" &&
					this.limiter.shouldHoldLowPriority()
				) {
					this.log.debug?.(
	...
	private bumpBucketReadyAt(priority: RequestPriority, readyAt: number): void {
		...
		this.bucketReadyAt.set(
			priority,
			typeof current === "number" ? Math.min(current, readyAt) : readyAt,
		);
	}
