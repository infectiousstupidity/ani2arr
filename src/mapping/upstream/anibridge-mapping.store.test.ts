...
// src/mapping/upstream/anibridge-mapping.store.test.ts

import { describe, expect, it, vi } from 'vitest';
import { parseAniListId } from '@/anilist';
import type { TtlCache, CacheHit, CacheWriteOptions } from '@/shared/cache/ttl-cache';
...
describe('AnibridgeMappingStore', () => {
  it('hydrates provider-aware cache payloads into forward and reverse indexes', async () => {
    ...
    const store = new AnibridgeMappingStore(cache, { fetch: async () => notModifiedResponse() });
    await store.init();
    ...
    expect(store.getAniListIdsForTvdb(parseTvdbId(424_536)).toSorted((left, right) => left - right)).toEqual([
      154_587,
      170_068,
      182_255,
    ]);
    expect(store.getAniListIdsForTmdb(parseTmdbId(12_345)).toSorted((left, right) => left - right)).toEqual([
      1001,
      1002,
    ]);
    expect(store.listAllProviderPairs().toSorted((left, right) => left.anilistId - right.anilistId)).toEqual([
