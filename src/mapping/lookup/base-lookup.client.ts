...
export abstract class BaseLookupClient<
	TResult extends ProviderLookupResult,
> implements ProviderLookupClient<ProviderCredentials, TResult> {
	...
	public async reset(): Promise<void> {
		this.inflight.clear();
		await Promise.all([
			this.caches.positive.clear(),
			this.caches.negative.clear(),
		]);
	}
	...
	public async lookup(
		...
		if (!safeTerm) {
			this.log.debug(
				`lookup: sanitized term empty for raw='${rawTerm}', skipping.`,
			);
			return [];
		}
			...
			try {
				if (!forceNetwork) {
					...
					if (positiveHit && !positiveHit.stale) {
						incrementCounter("mapping.lookup.cache_hit");
						this.log.debug(
							`lookup(${canonical}): returning fresh positive cache (deferred)`,
						);
					...
					if (negativeHit && !negativeHit.stale) {
						incrementCounter("mapping.lookup.negative_cache_hit");
						this.log.debug(
							`lookup(${canonical}): returning fresh negative cache (deferred)`,
						);
						deferred.resolve([]);
						return;
					}
	...
	public abstract getProviderId(result: unknown): ProviderId | null;
		...
		return timeAsync(
			...
			async () => {
				try {
					...
					this.log.debug(
						`performLookup: term='${term}' resultCount=${results.length}`,
					);
					return results;
				} catch (error) {
