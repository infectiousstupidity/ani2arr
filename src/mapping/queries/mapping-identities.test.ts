// src/mapping/queries/mapping-identities.test.ts
...
import {
	...
	type GetMappingIdentitiesDeps,
} from "./mapping-identities";
...
const createDeps = (input: {
	...
	manualMappingService: {
		get: ((provider, anilistId) =>
			(input.manuals ?? []).find(
				(entry) => entry.provider === provider && entry.anilistId === anilistId,
			)?.providerId ??
			null) as GetMappingIdentitiesDeps["manualMappingService"]["get"],
		isIgnored: (provider, anilistId) =>
			(input.ignores ?? []).some(
				(entry) => entry.provider === provider && entry.anilistId === anilistId,
			),
	...
	anibridgeMappingStore: {
		...
		getSonarrCandidates: (anilistId) =>
			(input.upstream ?? [])
				.filter(
					(
						entry,
					): entry is Extract<AnibridgeMappingPair, { provider: "sonarr" }> =>
						entry.provider === "sonarr" && entry.anilistId === anilistId,
				)
				.map((entry) => entry.providerId),
		getRadarrCandidates: (anilistId) =>
			(input.upstream ?? [])
				.filter(
					(
						entry,
					): entry is Extract<AnibridgeMappingPair, { provider: "radarr" }> =>
						entry.provider === "radarr" && entry.anilistId === anilistId,
				)
				.map((entry) => entry.providerId),
	},
	autoMappingStore: {
		get: async (provider, anilistId) =>
			(input.autoMappingRecords ?? []).find(
				(entry) => entry.provider === provider && entry.anilistId === anilistId,
			) ?? null,
