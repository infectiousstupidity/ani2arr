...
describe('ManualMappingService', () => {
  ...
  it('stores manual mappings and keeps linked AniList reverse IDs', async () => {
    ...
    expect(service.list('sonarr')).toEqual(expect.arrayContaining([
      expect.objectContaining({ anilistId: aid(10), provider: 'sonarr', providerId: tvdb(200) }),
      expect.objectContaining({ anilistId: aid(11), provider: 'sonarr', providerId: tvdb(200) }),
    ]));
    expect(service.getLinkedAniListIds('sonarr', tvdb(200))).toEqual([aid(10), aid(11)]);
  });
