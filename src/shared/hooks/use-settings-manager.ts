...
export function useSettingsManager() {
  ...
  const handleSave = useCallback(async () => {
    ...
    const newPermissionPatternResult = buildSonarrPermissionPattern(sonarrUrl);
    if (!newPermissionPatternResult.ok) {
      log.error('Failed to determine host permission for Sonarr URL.', newPermissionPatternResult.error);
    ...
    if (previousUrl && previousPermissionPatternResult && !previousPermissionPatternResult.ok) {
      log.error('Failed to determine host permission for previous Sonarr URL.', previousPermissionPatternResult.error);
      setSaveError('Failed to update host permissions. Please try again.');
      return;
    }

    const previousPermissionPattern =
      previousPermissionPatternResult && previousPermissionPatternResult.ok ? previousPermissionPatternResult.value : null;
