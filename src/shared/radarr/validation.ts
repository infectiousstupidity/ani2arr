...
export function buildRadarrPermissionPattern(input: string): Ok<string> | Err {
  return buildArrPermissionPattern(input);
}

export async function requestRadarrPermission(url: string): Promise<{ granted: boolean; error?: string }> {
  return requestArrPermission(url);
}

export async function hasRadarrPermission(url: string): Promise<boolean> {
